...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  private func startClickMonitoring() {
    ...
    clickMonitor = NSEvent.addLocalMonitorForEvents(matching: [
      .leftMouseDown, .rightMouseDown, .keyDown, .keyUp, .flagsChanged,
    ]) { [weak self] event -> NSEvent? in
      guard let self = self,
        let panelWindow = self.contentWindow as? ContentPanel
      else {
        return event
      }
  ...
  private func stopClickMonitoring() {
    ...
    NotificationCenter.default.removeObserver(
      self, name: NSWindow.didResignKeyNotification, object: contentWindow)
  }
  ...
  /// Calculate initial preview frame for a given item, centered on the card
  private func calculatePreviewFrame(for item: ClipboardItem, cardCenterX: CGFloat) -> NSRect {
    guard let contentWindow = contentWindow else {
      return NSRect(x: 100, y: 100, width: 500, height: 400)
    }
    ...
    // Position preview to overlap drawer header, arrow comes out of the card
    let previewY = drawerFrame.maxY - 80

    return NSRect(x: previewX, y: previewY, width: panelWidth, height: panelHeight)
  }

  func showPreviewWindow(for item: ClipboardItem, atIndex index: Int, cardCenterX: CGFloat) {
    // If preview is already showing, update position to center on new card
    if let existingPreview = previewWindow as? PreviewPanel, existingPreview.isVisible {
      let newFrame = calculatePreviewFrame(for: item, cardCenterX: cardCenterX)
      currentPreviewIndex = index
      existingPreview.updatePreview(item: item, frame: newFrame, arrowTargetX: cardCenterX)
      return
    }

    // First time showing - calculate initial position using the actual card center
    let frame = calculatePreviewFrame(for: item, cardCenterX: cardCenterX)
    currentPreviewIndex = index
    let arrowTargetX = cardCenterX
    ...
    previewPanel.finalizeSetup()  // Set up content view after frame is configured
    previewPanel.orderFront(nil)

    // Mark preview as visible in navigation state
    if let contentPanel = contentWindow as? ContentPanel {
      contentPanel.navigationState.isPreviewVisible = true
    }
  ...
  func closePreviewWindow() {
    ...
    // Mark preview as not visible in navigation state
    if let contentPanel = contentWindow as? ContentPanel {
      contentPanel.navigationState.isPreviewVisible = false
    }
  ...
  private func startPreviewClickMonitoring() {
    ...
    previewClickMonitor = NSEvent.addGlobalMonitorForEvents(matching: [
