...
enum QuickActionAnalyzer {
    ...
    private static let phonePattern = #"^[\+]?[(]?[0-9]{1,4}[)]?[\s.\-]?[(]?[0-9]{1,4}[)]?[\s.\-]?[0-9]{1,9}$"#
    ...
    /// Returns multiple matches when applicable (e.g. text that is both a URL and contains a color).
    ...
    static func actions(for item: ClipboardItem) -> [QuickAction] {
        ...
        for type in detected {
            ...
            case .url(let url):
                ...
                // Only offer clean URL if there are tracking params
                let cleaned = QuickActionAnalyzer.cleanURL(url)
                if cleaned.absoluteString != url.absoluteString {
                    actions.append(QuickAction(
                        title: "Copy Clean URL",
                        icon: "link.badge.plus",
                        action: { _ in
                            copyToPasteboard(cleaned.absoluteString)
                            QuickActionFeedback.shared.show("Copied!")
                        }
                    ))
                }
...
struct QuickActionsBar: View {
    ...
    /// Extract a color hex detection if present.
    private var colorInfo: (r: Int, g: Int, b: Int)? {
        for t in detectedTypes {
            if case .colorHex(_, let r, let g, let b) = t {
                return (r, g, b)
            }
        }
        return nil
